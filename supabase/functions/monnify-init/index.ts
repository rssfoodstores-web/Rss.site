...
const SITE_URL = (Deno.env.get("SITE_URL") ?? "https://myrss.com.ng").replace(/\/$/, "")
...
serve(async (request) => {
    ...
    try {
        ...
        if (metadata.type === "order_payment") {
            ...
            payload.paymentDescription = `Order ${order.id} payment`
            payload.redirectPath = `/account/orders/${order.id}`
            payload.metadata = {
                type: "order_payment",
                order_id: order.id,
            ...
            payload.paymentDescription = `Gift card ${giftCard.id} purchase`
            payload.redirectPath = `/account/gift-card?ref=${encodeURIComponent(giftCard.payment_reference)}`
            payload.metadata = {
        ...
        payload.customerName = typeof authData.user.user_metadata?.full_name === "string"
            ? authData.user.user_metadata.full_name
            : "RSS Foods Customer"
        payload.customerEmail = authData.user.email ?? "support@rssfoods.com"
        ...
        const initPayload = {
            ...
            redirectUrl: `${SITE_URL}${payload.redirectPath ?? "/account/orders"}`,
