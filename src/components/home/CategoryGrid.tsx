    ...
    return (
        <section className="container mx-auto px-4 md:px-8 py-12">
            <div className="flex items-center justify-between mb-8">
                <h2 className="text-2xl md:text-3xl font-bold text-gray-900 dark:text-white">Shop with Categories</h2>
            </div>

            <div className="flex overflow-x-auto snap-x py-4 gap-4 md:grid md:grid-cols-3 lg:grid-cols-6 md:gap-6 no-scrollbar">
                {categories.map((cat, idx) => (
                    <Link key={idx} href={`/category/${cat.name.toLowerCase().replace(' ', '-')}`} className="group min-w-[140px] md:min-w-0 snap-start first:pl-4 last:pr-4 md:first:pl-0 md:last:pr-0">
                        <div className="bg-white dark:bg-card rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-all border border-transparent hover:border-primary/20 h-full">
                            ...
                            <div className="p-4 text-center">
                                <h3 className="font-bold text-sm md:text-base group-hover:text-primary transition-colors">{cat.name}</h3>
